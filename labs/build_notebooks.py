...
"""),
        md("## 1. kit 과 어댑터 불러오기"),
        code(BOOTSTRAP + '\nimport lingua as L\nfrom compress import compress\nfrom kit.metrics import survival\n\ntable(\n    ["변형", "무엇이 다른가", "필요한 것"],\n    [["v1", "토큰별 정보량으로 프루닝", "인과 LM"],\n     ["long", "질문을 주고 문단별 중요도를 함께 봄", "인과 LM + 질문"],\n     ["v2", "분류 모델이 토큰을 남길지 판정", "전용 인코더"]],\n    align=["left", "left", "left"],\n    title="LLMLingua 3형제 — 같은 클래스, 다른 파라미터",\n)\n\n# 모델은 크기별로 고를 수 있습니다. 기본은 large 입니다.\ntable(\n    ["변형", "별칭", "모델", "크기"],\n    [[v, tier, name.split("/")[-1][:42], size]\n     for v, tiers in L.MODELS.items() for tier, (name, size) in tiers.items()],\n    align=["left", "left", "left", "left"],\n    title="고를 수 있는 모델",\n    note="기본은 large 입니다 — 이 랩의 결론이 모델 크기에 크게 좌우되어서, "\n         "작은 모델의 한계를 라이브러리 한계로 오해하지 않으시게 하려는 것입니다. "\n         "config 의 model_name 이나 --model 로 바꾸실 수 있습니다.",\n)\n'),

        md('\n## 1-1. 어느 모델로 돌릴지 고르기\n\n아래 셀을 실행하면 드롭다운이 나옵니다. **바꾸신 뒤 아래 셀들을 다시\n실행하면** 그 모델로 결과가 나옵니다.\n\n| 별칭 | `v1`·`long` | `v2` | 특징 |\n|---|---|---|---|\n| `small` | Qwen2.5-0.5B · 1GB | bert-base-multilingual · 700MB | 빠릅니다 |\n| `large` (기본) | Qwen2.5-1.5B · 3GB | **xlm-roberta-large · 2.2GB** | 정확합니다 |\n| `paper` | Llama-2-7b · 13GB | — | 논문이 쓴 것 |\n\n**처음 고르시는 모델은 내려받느라 몇 분 걸립니다.** `~/.cache/huggingface/`\n에 남아 다음부터는 로딩만 합니다. 기본값 `large` 는 합쳐서 5GB 남짓입니다.\n디스크나 시간이 부담되시면 `small` 로 바꾸셔도 이 랩은 그대로 돌아갑니다.\n\n### 쓰는 법 — 셀이 둘로 나뉘어 있습니다\n\n1. **첫 셀** — 드롭다운을 띄웁니다. 한 번만 실행하시면 됩니다.\n2. **둘째 셀** — 드롭다운의 **현재 값**을 읽어 확정합니다.\n\n값을 바꾸신 뒤에는 **둘째 셀부터** 다시 실행하세요. 한 셀에 합쳐 두면\n드롭다운을 바꿔도 계속 처음 값만 찍힙니다 — 셀을 다시 돌려야 값이 읽히기\n때문입니다.\n\n> 첫 셀을 다시 실행하시면 드롭다운이 기본값으로 초기화됩니다.\n\n> 드롭다운이 안 보이면 `ipywidgets` 가 없는 것입니다. 그때는 첫 셀의\n> `MODEL = "large"` 를 직접 고치셔도 똑같이 동작합니다.\n'),
        code('\n# 드롭다운을 띄웁니다. 값을 바꾸신 뒤 **아래 셀부터 다시 실행**하세요.\nMODEL = "large"          # small | large | paper | HuggingFace 경로\n\ntry:\n    import ipywidgets as W\n    from IPython.display import display\n\n    _sel = W.Dropdown(\n        options=[("large — 정확합니다 (기본)", "large"),\n                 ("small — 빠르고 가볍습니다", "small"),\n                 ("paper — 논문이 쓴 모델 (v1/long 만, 13GB)", "paper")],\n        value=MODEL, description="모델:",\n        style={"description_width": "initial"},\n        layout=W.Layout(width="420px"))\n    display(_sel)\n    print("고르신 뒤 아래 셀을 실행하시면 반영됩니다.")\nexcept ImportError:\n    _sel = None\n    print("ipywidgets 가 없습니다. 위의 MODEL 값을 직접 고치셔도 똑같이 동작합니다.")\n'),
        code('\n# 드롭다운의 **현재 값**을 읽습니다. 위 셀을 다시 실행할 필요가 없습니다.\ndef current_model():\n    return _sel.value if _sel is not None else MODEL\n\n\ndef model_for(variant):\n    """고른 모델을 이 변형에 맞게 풀어 줍니다.\n\n    v2 에는 paper 티어가 없습니다. 그때는 large 로 대신합니다 —\n    조용히 small 로 떨어지면 "큰 모델을 골랐는데 결과가 그대로" 가 됩니다.\n    """\n    m = current_model()\n    tiers = L.MODELS[variant]\n    if m in tiers:\n        return m\n    if m == "paper" and "large" in tiers:\n        return "large"\n    return m          # HuggingFace 경로를 직접 주신 경우\n\n\nMODEL = current_model()\nprint(f"MODEL = {MODEL!r}")\nfor v in ("v1", "long", "v2"):\n    picked = model_for(v)\n    note = "  ← paper 가 없어 large 로" if MODEL == "paper" and picked != MODEL else ""\n    print(f"  {v:5s} → {L.resolve_model(v, picked)}{note}")\nprint()\nprint("드롭다운을 바꾸신 뒤에는 이 셀부터 다시 실행하세요.")\n'),
...
"""),
        md('\n### 압축 결과를 직접 읽어보기\n\n표의 숫자는 **얼마나** 깨졌는지만 알려줍니다. **어떻게** 깨졌는지는 결과를\n직접 봐야 압니다. 숫자 중간이 잘렸는지, 문장이 통째로 빠졌는지, 아니면\n서식만 벌어졌는지에 따라 대응이 달라집니다.\n\n아래는 같은 사실을 담은 한·영 쌍을 **자르지 않고 전부** 출력합니다.\n정답 문자열이 남았으면 `✓`, 사라졌으면 `✗` 로 표시합니다.\n'),
        code('\n# ── 이 셀의 설정 ──────────────────────────────────────────────\nLOOK = ["ko-01", "en-01"]      # 보고 싶은 케이스. 다른 id 로 바꾸셔도 됩니다\nVARIANTS = ["v1", "long", "v2"]\n\n\ndef mark(text, must):\n    """정답 문자열이 남았는지 하나씩 표시합니다.\n\n    보존율과 같은 기준으로 봅니다 — 쉼표·공백을 지우고 비교하므로\n    \'32, 450, 000\' 도 \'32,450,000\' 이 남은 것으로 셉니다.\n    """\n    flat = text.replace(" ", "").replace(",", "")\n    return "  ".join(("✓ " if m.replace(" ", "").replace(",", "") in flat else "✗ ") + m\n                     for m in must)\n\n\nfor cid in LOOK:\n    probe = [c for c in cases if c.id == cid][0]\n    base = counter(probe.text)\n    print("=" * 78)\n    print(f"[{cid}] {probe.meta[\'lang\']} · 모델 {MODEL} · rate {RATE}")\n    print(f"질문: {probe.question}")\n    print("=" * 78)\n    print(f"\\n원문  {len(probe.text):,}자 · {base:,}토큰")\n    print(probe.text)\n\n    for v in VARIANTS:\n        out, _ = compress(probe.text, question=probe.question, variant=v,\n                          model_name=model_for(v), rate=RATE,\n                          force_reserve_digit=RESERVE_DIGIT)\n        t = counter(out)\n        print(f"\\n{\'─\' * 78}")\n        print(f"{v}  {base:,} → {t:,}토큰 · 절감 {1 - t / base:.1%} · "\n              f"보존 {survival(out, probe.must_include):.0%}")\n        print(f"  {mark(out, probe.must_include)}")\n        print(out)\n    print()\n'),
...
> **5절과 6절은 한 덩어리입니다.**
> 5절이 돌려서 결과를 `results` 에 쌓고, **6절이 그걸 표로 만듭니다.**
> 6절만 따로 실행하면 `results` 가 없어서 실패합니다.
>
> 나눠 둔 이유는 5절이 오래 걸리기 때문입니다. 표 모양만 고치고 싶으실
> 때 6절만 다시 돌리시면 됩니다.

> 여기서는 **설정 파일이 지정한 모델**을 씁니다. 위에서 고르신 `MODEL` 은
> 개별 셀(3·7절)에만 적용됩니다. 그래야 `v2` 와 `v2-large` 를 같은 표에서
> 비교할 수 있습니다.
...
"""),
        ...
        md(COMPARE_MD.format(n=6) + """
**5절을 먼저 실행하셔야 합니다.** 이 셀은 5절이 만든 `results` 를 읽기만
합니다. 다시 압축하지 않으므로 바로 끝납니다.

표에서 이렇게 확인해 보세요.

- 절감이 큰 조건이 보존율도 높은가, 아니면 맞바꾸는 관계인가
- 같은 변형에서 모델만 바꾸면(`v2` vs `v2-large`) 무엇이 달라지는가
- `rate=1.0` 인 `v2-noop` 의 절감이 0% 인가
"""),
...
> **이 값들은 추론으로 정한 것이지 측정한 값이 아닙니다.** 실험 설정의
> `protect` 로 바꿔가며 비교하실 수 있습니다
> (`experiments/protect-compare.yaml`).
