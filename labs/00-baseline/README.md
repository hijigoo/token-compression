...
재고, 둘이 얼마나 어긋나는지 알려줍니다.

| mode | 기준값 | 호출 | 언제 |
|---|---|---|---|
| **`both`** (기본) | API 실측 | 텍스트당 1회 | 과금 기준으로 재면서 추정 오차도 봅니다 |
| `api` | API 실측 | 텍스트당 1회 | 실측 값만 필요할 때 |
| `local` | tiktoken | 없음 | 네트워크·자격증명 없이 |

```bash
python compress.py configs/noop.yaml                    # both (기본)
python compress.py configs/noop.yaml --tokenizer local  # 호출 없이
python compress.py configs/noop-local.yaml              # 같은 뜻의 설정 파일
```

`both`로 돌리시면 이렇게 나옵니다.

```
절감 0.0% · 토큰 1,036 → 1,036
측정 방식 api:gpt-5.4+tiktoken:o200k_base
  API 실측 기준 + tiktoken 추정 병행 · 추정과 +7.5% 차이 · …
```

`+7.5%`는 **메시지 포맷 오버헤드**입니다. 케이스당 +6 토큰이 붙는데,
이 랩은 케이스가 짧아서 비율로는 크게 보입니다. 긴 문서에서는 작아집니다.

자격증명을 넣으시려면 이렇게 하시면 됩니다. 변수 이름이 탐색 노트북과
같아서, 이미 만들어 두신 것이 있다면 그대로 복사하셔도 됩니다.
...
`both`는 로컬 계산으로 내려가되 **왜 그랬는지 알려드립니다.** `api`는 같은
상황에서 예외를 냅니다 — 실측이 꼭 필요하다고 선언하신 것이라 조용히 다른
값을 드리면 안 되기 때문입니다.
...
| 어디서 | 무엇을 |
|---|---|
| 기본 실행 (`both`) | 케이스 12건의 압축 전후를 실측합니다 |
| 노트북 3번 절 | 같은 텍스트를 `local`과 `api`로 재서 비교합니다 (3회) |
| 노트북 마지막 절 | 과금 기준으로 절감률을 다시 계산합니다 (3건 × 2회) |

노트북 3번 절에서 이런 표를 보시게 됩니다.

```
텍스트                              길이   local   api   차이
환불 수수료는 결제금액의 10%입니다.     21자      16    22    +6
주문번호 A-1003 은 환불 완료…          26자      14    20    +6
제7조 환불 시 결제금액의 10%를…        90자      64    70    +6
```

길이와 상관없이 항상 `+6`입니다. 텍스트가 아니라 역할 구분자 같은 프레이밍이라
상수로 붙기 때문입니다.
...
하지 않아서 **압축 전과 후의 텍스트가 같기** 때문입니다.

강제로 다시 부르실 때는 `--refresh-tokens`를 붙이시면 됩니다.
