...
기본 설정(`noop.yaml`)은 `local`이라 **네트워크를 전혀 쓰지 않습니다.**
API는 두 곳에서만 탑니다.

| 어디서 | 무엇을 |
|---|---|
| 노트북 3번 절 | 같은 텍스트를 `local`과 `api`로 재서 비교합니다 (3회) |
| `noop-api.yaml` | 12건을 전부 실측합니다 (12회, 약 25초) |
...
길이와 상관없이 항상 `+6`입니다. 이게 **메시지 포맷 오버헤드**입니다.
텍스트가 아니라 역할 구분자 같은 프레이밍이라 상수로 붙습니다.
